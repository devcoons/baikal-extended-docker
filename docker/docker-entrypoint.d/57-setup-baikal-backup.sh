...
#
#   BAIKAL_BACKUP_ENABLED    "true" to enable (default: true)
#   BAIKAL_BACKUP_CRON       cron schedule (default: "0 3 * * *" = daily 03:00)
#   BAIKAL_BACKUP_DIR        output dir (default: <Specific>/backups)
#   BAIKAL_BACKUP_KEEP       archives to retain (default: 7)
#   BAIKAL_BACKUP_RUN_ON_START  "true" to also back up shortly after boot
set -e
ME=$(basename "$0")

ENABLED="${BAIKAL_BACKUP_ENABLED:-true}"
CRON_FILE="/etc/cron.d/baikal-backup"

case "$ENABLED" in
    1|true|TRUE|yes|on) : ;;
    *)
        echo "$ME: backups disabled (BAIKAL_BACKUP_ENABLED=$ENABLED)"
        rm -f "$CRON_FILE"
        exit 0
        ;;
esac
...
BACKUP_DIR="${BAIKAL_BACKUP_DIR:-$SPECIFIC/backups}"
mkdir -p "$BACKUP_DIR"
...
SCHEDULE="${BAIKAL_BACKUP_CRON:-0 3 * * *}"

{
    ...
    for var in BAIKAL_HOME BAIKAL_EXT_HOME BAIKAL_PATH_CONFIG BAIKAL_PATH_SPECIFIC \
               BAIKAL_BACKUP_DIR BAIKAL_BACKUP_KEEP; do
        eval "value=\${$var:-}"
        if [ -n "$value" ]; then
            echo "$var=$value"
        fi
...
case "${BAIKAL_BACKUP_RUN_ON_START:-false}" in
    1|true|TRUE|yes|on)
